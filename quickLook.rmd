...
mylogit <- glm(same ~ group, data = scorps, family = "binomial")
summary(mylogit)
```

### Making sense out of the results

So logit=-2.0369 +2.1322*groupmarble + 2.3470*groupnosePainted + .6506*groupuntreated
...
We can get a conf interval with the given standard error of .7301 adding and subtracting 2 times the standard error
gives (.3101 - 2*.7301) = -1.1501  and (.3101 + 2*.7301)= 1.7703
so odds are exp(-1.1501) = 0.3166051  to exp(1.7703) = 5.872

somewhat tighter than the fisher exact test

see if you can work out a 95% confidence interval for the odds given it was in the untreated group

How about the footPainted group (trick question)
